...
export class DefaultDiagramNode extends DiagramNode {
    ...
    constructor(logicalId: string, name: string, type: string, x: number, y: number, width: number, height: number,
    ...
    private subscribeJointEventsToSystemEvents(): void {
        this.jointObject.on('transition:end', function () {
            ChangeElementEvent.signalEvent()
        });
        this.jointObject.on('change:size', function () {
            ChangeElementEvent.signalEvent()
        });
        this.jointObject.on('change:attrs', function () {
            ChangeElementEvent.signalEvent()
        });
    }
